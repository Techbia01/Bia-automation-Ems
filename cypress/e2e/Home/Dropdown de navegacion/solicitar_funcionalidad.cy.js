...
describe('Dropdown de navegación - Solicitar funcionalidad', () => {
  let loginPage;
  let homePage;
  ...
  it('Debería redirigir a Typeform al solicitar funcionalidad', () => {
    ...
    // Paso 2: Solicitar funcionalidad
    ...
    cy.log('✅ Caché limpiada, continuando con solicitar funcionalidad...');

    // Hacer clic en el botón sidebar header
    ...
    // El elemento es un div con onClick, no un enlace tradicional
    // Verificar que el elemento existe y es visible
    cy.get('#request-feature', { timeout: 10000 })
      .should('exist')
      .should('be.visible')
      .then(($el) => {
        cy.log(`🔍 Elemento encontrado: ${$el[0].tagName} con ID ${$el.attr('id')}`);
        cy.log(`🔍 Clases del elemento: ${$el.attr('class')}`);
      });

    // Hacer clic en el botón "solicitar funcionalidad"
    cy.get('#request-feature', { timeout: 10000 })
      .should('be.visible')
      .click();

    cy.log('✅ Clic realizado en el botón "solicitar funcionalidad"');
    cy.log('✅ El botón debería haber abierto una nueva pestaña con el formulario de Typeform');
    
    cy.log('✅ Solicitud de funcionalidad completada exitosamente - Test finalizado');
  });
});
